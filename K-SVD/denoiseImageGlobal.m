function [IOut,output] = denoiseImageGlobal(D, Image, sigma, varargin)
%==========================================================================
...
for jj = 1:10000:size(blocks,2)
    ...
    Coefs = OMPerr(D,blocks(:,jj:jumpSize),errT,vecOfMeans);
    if (Reduce_DC)
...
for i = 1:size(blocks,2)
    blocks(:,i) = (blocks(:,i)-min(blocks(:,i))) / (max(blocks(:,i)) - min(blocks(:,i)));
end
...
output.D = D;
% IOut = (Image*0.5+0.15*sigma*IMout)./(0.5+0.15*sigma*Weight);
...
% figure;
% subplot(221); imshow(IMout,[]);  title('IMout'); colorbar;
% subplot(222); imshow(Weight,[]); title('Weight'); colorbar;
% subplot(223); imshow(IOut); title('IOut'); colorbar;
% subplot(224); imshow(IOut_Ori); title('IOut_Ori');colorbar;
% figure;
% subplot(1,2,1); imshow(IOut_Ori_S); title('IOut_Ori_S');colorbar;
% subplot(1,2,2); mesh(IMout); title('IMout');
