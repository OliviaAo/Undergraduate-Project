function [IOut,output] = denoiseImageKSVD(Dic,Image,sigma,K,varargin)
%==========================================================================
...
if(prod([NN1,NN2]-bb+1)> maxNumBlocksToTrainOn)
    ...
    randPermutation =  randperm(prod([NN1,NN2]-bb+1));
    selectedBlocks = randPermutation(1:size(blkMatrix,2));
end
...
for jj = 1:10000:size(blocks,2)
    ...
    Coefs = OMPerr(Dictionary,blocks(:,jj:jumpSize),errT,vecOfMeans);
%     disp(['KSVD-Coefs:',num2str(length(find(Coefs))/maxNumBlocksToTrainOn)]);
    if (reduceDC)
...
% figure;
% I = displayDictionaryElementsAsImage(Dictionary, floor(sqrt(K)), floor(size(Dictionary,2)/floor(sqrt(K))),bb,bb);
