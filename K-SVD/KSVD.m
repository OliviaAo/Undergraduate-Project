...
for iterNum = 1:param.numIteration
    ...
    if (param.errorFlag==0)
        ...
        vecOfMeans = mean(Data(:,1:size(Data,2)));
%         CoefMatrix = OMPerr([FixedDictionaryElement,Dictionary],Data, param.errorGoal);
        CoefMatrix = OMPerr([FixedDictionaryElement,Dictionary],Data, param.errorGoal,vecOfMeans);
        param.L = 1;
%         disp(['Hello????']);
    end
%      disp(['length(find(CoefMatrix)): ',num2str(length(find(CoefMatrix))),' find(CoefMatrix): ',num2str(find(CoefMatrix))]);
     
    replacedVectorCounter = 0;
	rPerm = randperm(size(Dictionary,2));
    
%     for i = 1:param.numIteration
        for j = rPerm
...
%             Dictionary = mapminmax(Dictionary, 0, 1);
%             CoefMatrix = mapminmax(CoefMatrix, 0, 1);
            if (param.preserveDCAtom)
...
%      K = size( Dictionary,2);
%      bb = sqrt(K / 4);
%      I = displayDictionaryElementsAsImage(Dictionary, floor(sqrt(K)), floor(size(Dictionary,2)/floor(sqrt(K))),bb,bb,0);
%      title( 'after findBetterDictionaryElement');
    ...
    if (iterNum>1 & param.displayProgress)
        if (param.errorFlag==0)
            ...
            disp(['length(find(CoefMatrix))   ',num2str(length(find(CoefMatrix))),' size(Data,2) ',num2str(size(Data,2))]);
...
relevantDataIndices = find(CoefMatrix(j,:)); % the data indices that usewis the j'th dictionary element.
